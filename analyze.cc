...
struct Pairing
{
    ...
    bool contains(const Couple c) const
    {
        return women[c.first] == c.second;
    }
    ...
    void print() const
    {
        ...
        {
            printf("(%s,%s)", MAN_MAP.at(i).c_str(), WOMAN_MAP.at(women[i]).c_str());
        }
        printf("\n");
    }
...
void printCoupleCounts(const Pairing *pairings, const bool *pairing_valid, bool dropZeros)
{
